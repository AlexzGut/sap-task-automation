...
def setup_logging() -> logging.Logger:
    ...
    if getattr(sys, 'frozen', False):
        ...
        log_dir = (os.path.join(os.path.dirname(sys.executable), '_internal', 'retrieve_sap_statement', 'logs'))
    else:
        log_dir = (os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'logs'))

    log_file = 'retrieve_last_statement.log'
...
def access_tcode_fbl5n(session) -> None:
    ...
    # Company code from SDM: L600, MediSystem: L604(West), L607(East)
    session.findById("wnd[0]/usr/ctxtDD_BUKRS-LOW").text = "L600" 
